...
from core.olt import (
    get_olt_connection, close_all, OntNotFoundError, OltConnection
)
...
def _parse_address(ont: str) -> dict:
    ...
    if re.fullmatch(r"^(fl_)?\d{5,16}$", ont) or (ont.isdigit() and 5 <= len(ont) <= 16):
        value = ont
        if ont.isdigit():
            value = f"fl_{ont}"
...
async def _tool_connect(params: dict) -> list[types.TextContent]:
    ...
    try:
        ...
        olt.connect()
        # get_olt_info() disabled due to MA5608T param prompt issue
        # info = olt.get_olt_info()
        return [types.TextContent(type="text", text=json.dumps({
            "status": "connected",
            "session": session_name,
            "host": host,
            "olt_info": {"host": host, "port": port, "status": "online"}
        }, ensure_ascii=False))]
